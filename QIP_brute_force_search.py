...
def best_dist_dual(n, m, inter_cand_dist_vals, weight_vals):
    ...
    # Constraints in QCIP
    for c in C:
        model.addConstr(psi <= sum(d[c, v, o] * w[v] for o in C for v in V))

    model.addConstr(sum(nu[o] for o in C) <= 1)
    for o in C:
        model.addConstr(sum(d[o, v, o] * w[v] for v in V) <= nu[o])
    ...
    wt = model.addVars(V, weight_vals, vtype=gp.GRB.BINARY)
    for v in V:
